...
const GameApp: React.FC = () => {
  ...
  useEffect(() => {
    if (game.state.playerName && currentScreen === 'welcome') {
      setCurrentScreen('map');
    }
  }, [game.state.playerName, currentScreen]);

  useEffect(() => {
    if (!game.state.playerName) return;
  ...
  }, [game.state.playerName, currentScreen, currentLevelId, gameOverScore, isBossGameOver, challengeLevelId, pendingPerfectChallenge]);

  useEffect(() => {
    const syncCloud = async () => {
      if (!navigator.onLine || !isCloudSyncConfigured() || !getAuthSession() || !game.state.playerName) return;
  ...
  const handleCloudLogin = async (mode: 'login' | 'register') => {
    ...
    try {
      ...
      if (cloudData?.game?.data?.playerName) {
        game.setPlayerInfo(cloudData.game.data.playerName, cloudData.game.data.avatar || '🧙');
      }
  ...
  const handleBossComplete = (timeRemaining: number = 0) => {
    game.completeBoss(timeRemaining);
    setCurrentScreen('map');
  };
      ...
      case 'domain_challenge': {
        ...
        return (
          <DomainChallengeScreen
            ...
            onComplete={() => {
              game.completeLevel(challengeLevelId, pendingPerfectChallenge);
