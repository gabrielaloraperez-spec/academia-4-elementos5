...
export const StoryCard: React.FC<StoryCardProps> = ({ title, text, onClick, isVisible, isFinal = false }) => {
  return (
    <button
      ...
      onClick={onClick}
      className={[
        'w-full max-w-3xl rounded-3xl border border-amber-200/40 bg-[linear-gradient(135deg,rgba(251,243,209,0.93),rgba(255,247,226,0.92))]',
    ...
    >
      ...
      <p className="mt-4 text-base md:text-xl text-amber-950/85 whitespace-pre-line leading-relaxed">{text}</p>

      <div className="mt-8 text-sm md:text-base text-amber-800/80 font-semibold flex items-center justify-between">
        {isFinal ? <span>✨ El destino te espera</span> : <span>✨ Click para continuar</span>}
