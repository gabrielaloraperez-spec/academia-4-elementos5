...
export const DomainChallengeScreen: React.FC<DomainChallengeScreenProps> = ({ level, onComplete, onFail }) => {
  ...
  if (!level || mixedProblems.length === 0) {
    return (
      <div className="min-h-screen w-full bg-cover bg-center flex items-center justify-center p-4" style={{ backgroundImage: `linear-gradient(rgba(30,41,59,0.68), rgba(30,41,59,0.52)), ${withSvgFallback(challengeTheme?.background ?? '/assets/backgrounds/world-map.png')}` }}>
  ...
  return (
    <div className="min-h-screen w-full bg-cover bg-center p-4" style={{ backgroundImage: `linear-gradient(rgba(30,41,59,0.68), rgba(30,41,59,0.52)), ${withSvgFallback(challengeTheme?.background ?? '/assets/backgrounds/world-map.png')}` }}>
